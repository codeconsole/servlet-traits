if [ $# -lt 1 ] || { [ "$1" != "jakarta" ] && [ "$1" != "javax" ]; } then
...
groovyc -cp lib/$servlet $src/Person.groovy -d build

echo Moving ServletAttributes* ....
mv build/ServletAttributes*.class dist

echo Running $src with $servlet and $groovy ....
java -cp dist:lib/$groovy:lib/$servlet Employee